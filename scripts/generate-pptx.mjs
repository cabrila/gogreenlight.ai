...
pptx.theme = { headFontFace: FONT_TITLE, bodyFontFace: FONT_BODY };

// Helper: add dark background + subtle grid lines to every slide
...
// Helper: gradient-style title text (uses accent color)
...
// Helper: bullet point dot
function addBullet(slide, text, x, y, w) {
  slide.addShape(pptx.ShapeType.ellipse, {
    x: x - 0.25, y: y + 0.08, w: 0.1, h: 0.1,
...
// ─── SLIDE 6: MOVIELABS ───────────────────────────────────────────────────────
{
  ...
  pts.forEach((pt, i) => {
    ...
    slide.addShape(pptx.ShapeType.roundRect, {
      ...
      rectRadius: 0.06,
      fill: { color: "1A3D24" }, line: { type: "none" },
    });
    slide.addText(`${i + 1}`, {
      x: 1.2, y: y + 0.28, w: 0.42, h: 0.42,
  ...
  addBadge(slide, "OMC v2.8 Native", 5.57, 6.05);
}
...
// ─── SLIDE 7: THE MOAT ────────────────────────────────────────────────────────
{
  ...
  addBadge(slide, "Defensible by Design", 5.57, 6.2);
}
...
// ─── WRITE FILE ───────────────────────────────────────────────────────────────
await pptx.writeFile({ fileName: "GoGreenlight-PitchDeck.pptx" });
